...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
	if (request.max == 'MAX') {
		let doc = document.getElementsByClassName("money")
		let moneyList = [...doc].map((item, index) => +item.innerHTML.replace(/[$USD]/g, ""))
		console.log(moneyList, 666);
		let MAX = Math.max(...moneyList)
		let MIN = Math.min(...moneyList)
		console.log();
		sendResponse(JSON.stringify({ MIN, MAX }))
	}
	else {
